  ...
  window.Site = Singleton.create({
    ...
    bindUI: function() {
      ...
      if (Y.one('body.collection-type-index')) {
        ...
        this.mutationObserver(Y.one('#content-wrapper').getDOMNode(), { childList: true, subtree: true }, this.mutationCallback);
    ...
    setupMobileLayout: function () {
      ...
      Y.all('.parallax-item').each(function (item, i) {
        ...
        if (wrapper.hasClass('has-main-image')) {
          wrapper.setStyles({
            height: i === 0 ? this.INITIAL_WINDOW_HEIGHT - headerHeight : imageHeight,
            overflow: 'hidden'
          }, this);

          image.setStyles({
            height: i === 0 ? imageHeight - 118 : imageHeight
          });
        } else {
          wrapper.setStyle({
      ...
      if (!Y.one('.parallax-scrolling') || (Y.UA.ie > 0 && Y.UA.ie <= 9)) {
        ...
        Y.all('.title-desc-image').each(function (image, i) {
          if (i === 0) {
            image.setStyle('height', Y.config.win.innerHeight);
          } else {
    ...
    setScrollArrowColor: function () {
      ...
      if (Y.Lang.isValue(scrollArrow) && Y.Lang.isValue(colorDetect)) {
        scrollArrow
          ...
          .addClass('color-weight-' + this._getLightness(colorDetect));
      }
    },
